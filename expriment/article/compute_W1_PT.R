...
# empirical CDF of a vector, conditioned on being in [a,b]
emp_cdf_trunc_from_vector <- function(v, a, b, xg) {
  s <- as.numeric(v)
  s <- s[is.finite(s)]
  inb <- (s >= a) & (s <= b)
  s_in <- s[inb]

  if (length(s_in) == 0) return(rep(NA_real_, length(xg)))

  # F(x) = P(S <= x | S in [a,b])
  vapply(xg, function(x) mean(s_in <= x), numeric(1))
}
...
F_true <- make_trunc_cdf_from_pdf(true_pdf, a_trunc, b_trunc, n_grid)
...
w1_pt_vals  <- numeric(0)
w1_lse_vals <- numeric(0)
...
for (d in run_dirs) {
  ...
  w1_pt  <- W1_on_grid(xg, F_pt_g, F_true_g)
  w1_pt_vals <- c(w1_pt_vals, w1_pt)

  # ---- LSE ----
  if (!is.null(sc$beta_lse)) {
    F_lse_g <- emp_cdf_trunc_from_vector(sc$beta_lse, a_trunc, b_trunc, xg)
    w1_lse  <- W1_on_grid(xg, F_lse_g, F_true_g)
  } else {
    w1_lse <- NA_real_
  }
  w1_lse_vals <- c(w1_lse_vals, w1_lse)

  message(sprintf("%s | W1(PT,true)=%.6g | W1(LSE,true)=%.6g",
                  basename(d), w1_pt, w1_lse))
}


cat(sprintf("\nPT W1 over %d runs:  mean = %.6g, sd = %.6g\n",
            length(w1_pt_vals), mean(w1_pt_vals), sd(w1_pt_vals)))

ok_lse <- is.finite(w1_lse_vals)
cat(sprintf("LSE W1 over %d runs (finite only): mean = %.6g, sd = %.6g\n",
            sum(ok_lse), mean(w1_lse_vals[ok_lse]), sd(w1_lse_vals[ok_lse])))
